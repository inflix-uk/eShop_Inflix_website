...
export default function BookingPageClient({
  ...
  // Show all active public packages (including editing), matching live booking grid.
  const listablePackages = packages;

  const filteredPackages =
    selectedType === "all"
      ? listablePackages
      : listablePackages.filter((pkg) => pkg.type === selectedType);

  const packageTypes = [
    "all",
    ...Array.from(new Set(listablePackages.map((p) => p.type))),
  ];
                ...
                {filteredPackages.flatMap((pkg, index) => {
                  const nodes = [
                    <BookingPackageCard
                      key={pkg._id}
                      pkg={pkg}
                      studioMicCapacity={settings?.studioMicCapacity ?? 5}
                    />,
                  ];
