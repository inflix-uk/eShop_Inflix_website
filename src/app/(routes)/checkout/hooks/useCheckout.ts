...
export const useCheckout = () => {
  ...
  // Shipping rates for the wallet (Apple Pay / Google Pay). Without these
  // Google Pay errors out with OR_BIBED_06 before Stripe's onConfirm fires.
  ...
  // When user picks a shipping rate inside the wallet UI, sync the
  // server-side PaymentIntent amount so it matches what the wallet charges.
  const handleExpressShippingRateChange = useCallback(
    async (rateId: string): Promise<{ success: boolean }> => {
      ...
      if (!storedPaymentIntentId) {
        logCheckoutEvent({ event: 'frontend.wallet.rate_change.no_payment_intent', data: { rateId } });
        return { success: false };
      }
  ...
  // fires with empty metadata and the backend webhook cannot match an order.
  const validateAndCreateOrderFromWallet = useCallback(
    async (expressCheckoutData: any): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        // Prefer wallet-picked rate; fall back to hook's selected method.
        ...
        try {
          ...
          // Metadata patch MUST succeed — otherwise webhook fires without
          // orderNumber and the order can never be marked Pending. Abort.
          logCheckoutEvent({
